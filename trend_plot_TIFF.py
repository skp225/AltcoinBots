...
try:
    ...
    def plot_trends(trending_projects, period):
        ...
        for i in range(0, len(trending_projects), projects_per_image):
            ...
            tiff_file = os.path.join(graphs_dir, f'upward_trends_{period}_days_{current_date}_part{files_created + 1}.tiff')
            plt.savefig(tiff_file, dpi=300, bbox_inches='tight', format='tiff')
            plt.close(fig)
            
            logger.info(f"Facet graph for {period}-day upward trends (Part {files_created + 1}) saved as '{tiff_file}'")
            files_created += 1
    ...
    message = f"Charts available! Total TIFF files generated: {total_files}. Check the Graphs folder for multiple parts for each trend period."
    ...
    send_telegram_message(f"An error occurred in trend_plot_TIFF.py: {str(e)}")
    sys.exit(1)
